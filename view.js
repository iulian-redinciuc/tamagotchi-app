function createPETDIV(myModel, petName) {
        ...
        pets.notify = function(petsList) {
            ...
            petsList.forEach(function(pet) {
                ...
                foodStatus.innerHTML = `<div class="status">Food </div><progress id="${pet.getAttr('name')}-food" value="${pet.getAttr('food')}" max="100"></progress><br>`;
               
                let waterStatus = document.createElement("div");
                waterStatus.innerHTML = `<div class="status">Water </div><progress id="${pet.getAttr('name')}-water" value="${pet.getAttr('water')}" max="100"></progress><br>`
               
                let funStatus = document.createElement("div");
                funStatus.innerHTML = `<div class="status">Fun </div><progress id="${pet.getAttr('name')}-fun" value="${pet.getAttr('fun')}" max="100"></progress><br>`;
               
                let fatigueStatus = document.createElement("div");
                fatigueStatus.innerHTML = `<div class="status">Fatigue </div><progress id="${pet.getAttr('name')}-fatigue" value="${pet.getAttr('fatigue')}" max="100"></progress><br>`;
                
                let levelStatus = document.createElement("div");
                levelStatus.innerHTML = `<div class="status">Level </div><progress id="${pet.getAttr('name')}-level" value="${pet.getAttr('level')}" max="100"></progress>`;
                
                let actions = document.createElement("div");
                actions.classList.add("call-to-actions");
                actions.innerHTML = `
                <button id="${pet.getAttr('name')}-feed">Feed</button>
                <button id="${pet.getAttr('name')}-give-water">Give Water</button>
